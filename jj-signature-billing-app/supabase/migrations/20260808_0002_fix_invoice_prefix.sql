-- Fix: Add INV prefix back to invoice number generation
...
VOLATILE
AS $$
  SELECT 'INV' || LPAD(nextval('public.invoice_number_seq')::TEXT, 8, '0');
$$;

-- Update complete_advance_order_v2 to also use INV prefix
...
BEGIN
  ...
  -- Generate invoice number using the existing sequence WITH INV PREFIX
