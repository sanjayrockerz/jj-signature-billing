...
-- The earlier migration returns a single advance_orders row. PostgreSQL cannot
-- change a function's return type through CREATE OR REPLACE, so remove the
-- exact existing signature before recreating it as SETOF advance_orders.
