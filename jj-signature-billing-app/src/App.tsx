...
function LoadingSpinner() {
  return (
    <div className="flex min-h-screen items-center justify-center bg-bgMain">
      <span className="h-10 w-10 animate-spin rounded-full border-4 border-[#D8CBB7] border-t-[#111111]" />
    </div>
  )
}
...
function AppShell() {
  ...
  return (
    <div className="min-h-screen w-full max-w-[100vw] overflow-x-hidden bg-bgMain print:block print:min-h-0 print:overflow-visible">
      <main className="min-h-screen print:block print:overflow-visible">
        <Routes>
          ...
          <Route path="/expenses" element={<AdminGuard><Suspense fallback={<LoadingSpinner />}><ExpenseTracker /></Suspense></AdminGuard>} />
          <Route path="/inventory" element={<AdminGuard><Suspense fallback={<LoadingSpinner />}><InventoryProducts /></Suspense></AdminGuard>} />
          <Route path="/categories" element={<Navigate to="/dashboard?tab=categories" replace />} />
          <Route path="/order-history" element={<Navigate to="/dashboard?tab=history" replace />} />
          <Route path="/analytics" element={<Navigate to="/pos-analytics" replace />} />
          <Route path="/coupons" element={<Navigate to="/dashboard?tab=coupons" replace />} />
          <Route
            path="/pos"
            element={
              <PosGuard>
