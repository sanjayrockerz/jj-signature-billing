...
export async function advanceReceiptPdf(order: AdvanceOrder) {
  ...
  doc.setFillColor('#CBB89D'); doc.rect(0, 0, 210, 5, 'F')
  doc.setTextColor('#111111'); doc.setFont('helvetica', 'bold'); doc.setFontSize(18); doc.text(BRAND_EN.toUpperCase(), 36, 20)
  if (logoData) doc.addImage(logoData, 'PNG', 16, 7, 14, 14)
  ...
  doc.setDrawColor('#CBB89D'); doc.line(16, 38, 194, 38)
  ...
  const rows = [
    ['Customer', order.customer_name], ['Phone', formatIndianPhone(order.phone)], ['Address', order.address || '-'], ['Product', order.product_name],
  ...
  y += 4; doc.setFillColor('#F3EBDD'); doc.roundedRect(16, y, 178, 42, 3, 3, 'F')
  ...
  money.forEach(([label, value], index) => { const rowY = y + 11 + index * 11; doc.setFont('helvetica', index === 2 ? 'bold' : 'normal'); doc.setTextColor(index === 2 ? '#171717' : '#625A50'); doc.text(label, 22, rowY); doc.text(formatCurrency(value), 188, rowY, { align: 'right' }) })
  doc.setFont('helvetica', 'bold'); doc.setTextColor('#625A50'); doc.setFontSize(9); doc.text('This receipt records an advance payment only. It is not a final invoice.', 105, y + 55, { align: 'center' })
