...
export default function AddProductModal({ isOpen, onClose, onSuccess }: AddProductModalProps) {
  ...
  const [categoryOptions, setCategoryOptions] = useState<Array<{ id: string | number; name_en: string }>>([])
  ...
  const existingCategories = categoryOptions.length > 0
    ? categoryOptions.map(category => category.name_en)
  ...
  useEffect(() => {
    ...
    const loadCategories = async () => {
      const { data } = await supabase
        ...
        .eq('is_active', true)
        .order('sort_order')
      if (!cancelled) setCategoryOptions((data || []).map(row => ({
        id: row.id,
