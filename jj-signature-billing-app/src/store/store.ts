  ...
  persist(
    (set) => ({
      ...
      login: async (portalId: string, password: string) => {
        ...
        const role = profile?.role === 'admin' ? 'admin' : profile?.role === 'staff' ? 'staff' : null
        if (!role) { await supabase.auth.signOut(); return false }
        set({ isLoggedIn: true, role })
        return role
      },
      logout: () => { void supabase.auth.signOut(); set({ isLoggedIn: false, role: null }) },
    }),
    {
