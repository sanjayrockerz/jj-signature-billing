...
type AdminNavItem = { id: TabKey; icon: React.ReactNode; label: string; route?: never } | { id: 'expenses' | 'inventory'; icon: React.ReactNode; label: string; route: string }
...
type DashboardExpense = { expense_date: string; amount: number }
...
const exportCSV = (orders: DashboardOrder[]) => {
  const header = ['Order Ref', 'Customer', 'Country Code', 'Mobile Number', 'Date', 'Total (₹)', 'Order Type', 'Status']
  const rows = orders.map(o => [
    o.order_type === 'online_request' ? o.id : o.invoice_no, o.customer_name,
    getIndianPhoneParts(o.phone).countryCode, getIndianPhoneParts(o.phone).mobileNumber,
    new Date(o.created_at).toLocaleDateString('en-IN'),
    getOrderTotal(o).toFixed(2), o.order_type, o.status,
  ])
  const csv = [header, ...rows].map(r => r.map((c, index) => {
    const value = String(c).replace(/"/g, '""')
    return index === 2 || index === 3 ? `="${value}"` : `"${value}"`
  }).join(',')).join('\n')
...
export default function Dashboard() {
  ...
  const [tab, setTab] = useState<TabKey>(() => {
    ...
    if (requestedTab === 'categories' || requestedTab === 'history' || requestedTab === 'coupons') return requestedTab
  ...
  const analytics = useMemo(() => {
    ...
    const averageRevenuePerBill = billableCompleted.length > 0 ? netRevenue / billableCompleted.length : 0
    ...
    const statusDistribution = [
      { name: 'WA Requests', value: waOrders.length, color: '#CBB89D' },
      { name: 'POS Pending', value: pendingOrders.filter(o => normalizeOrderType(o.order_type) !== 'online_request').length, color: '#f59e0b' },
      { name: 'Completed',   value: billableCompleted.length, color: '#8C7358' },
    ]
    const channelDistribution = [
      { name: 'Offline Bills', value: posRevenue, color: '#A68B68' },
      { name: 'Online Bills',  value: onlinePosRevenue, color: '#D8CBB7' },
      { name: 'Manual Sales',  value: manualRevenue || totalManualRevenue, color: '#6B6258' },
    ]
    ...
    return {
      ...
      grossSalesRevenue: completedRevenue,
      totalExpenses,
  ...
  const saveCoupon = async (e: FormEvent) => {
    ...
    if (error) {
      ...
      if (msg.toLowerCase().includes('row-level security')) {
        setCouponSaveError('Coupon save was blocked by Supabase RLS. Confirm your user has the admin role and apply the J.J Signature migrations.')
      } else {
  ...
  const allNavItems: AdminNavItem[] = [
    ...
    { id: 'inventory',    icon: <Boxes size={20} />,             label: 'Inventory & Products', route: '/inventory' },
      ...
      >
        ...
        >
          {navItems.map(item => (
            <button
              key={item.id}
              onClick={() => item.route ? navigate(item.route) : handleTabClick(item.id as TabKey)}
              title={item.label}
              className={[
            ...
            >
              ...
              <span className={`block truncate text-center lg:text-left transition-all duration-200 ${sidebarCollapsed ? 'lg:w-0 lg:opacity-0 lg:overflow-hidden' : 'opacity-100 lg:flex-1'}`}>
                {item.label}
              </span>
            </button>
          ))}
          ...
          >
            ...
            <span className={`block truncate text-center lg:text-left transition-all duration-200 ${sidebarCollapsed ? 'lg:w-0 lg:opacity-0 lg:overflow-hidden' : 'opacity-100 lg:flex-1'}`}>Logout</span>
          </button>
        </nav>
                    ...
                    <div className="h-48">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={analytics.monthlyTrend}>
                          ...
                          <Bar dataKey="revenue" fill="#CBB89D" radius={[4, 4, 0, 0]} barSize={12} />
                        </BarChart>
                      </ResponsiveContainer>
                    </div>
                  </div>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  {[
                    ...
                    { label: 'Average Product Revenue', value: `${formatCurrency(analytics.averageProductRevenue)} / Product`, icon: <RupeeIcon size={18} />, from: 'from-violet-500 to-purple-600' },
