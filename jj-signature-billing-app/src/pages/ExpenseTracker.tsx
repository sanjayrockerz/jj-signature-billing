...
export default function ExpenseTracker() {
  ...
  return <AdminModuleShell title="Expense Tracker" subtitle="Record and understand every operating expense from the J.J Signature database.">
    ...
    {tab === 'expenses' ? <>
      <StatGrid className="mb-5">{[['Today', totals.today], ['This Week', totals.week], ['This Month', totals.month], ['This Year', totals.year], ['Total All Time', totals.all]].map(([label, value]) => <StatCard key={String(label)} label={label} value={formatCurrency(Number(value))} className="bg-cardBg shadow-soft" labelClassName="text-textMuted" valueClassName="text-lg text-[#171717]" />)}</StatGrid>
      <div className="mb-5 flex flex-wrap items-end gap-2 rounded-2xl border border-borderLight bg-cardBg p-3">{(['all', 'today', 'week', 'month', 'year', 'custom'] as Filter[]).map(value => <button key={value} onClick={() => setFilter(value)} className={`rounded-lg px-3 py-2 text-xs font-black capitalize ${filter === value ? 'bg-[#CBB89D] text-[#171717]' : 'border border-borderLight text-textMuted hover:bg-[#EDE4D4]'}`}>{value === 'all' ? 'All Time' : value === 'custom' ? 'Custom' : `This ${value}`}</button>)}{filter === 'custom' && <><label className="text-sm font-bold text-textMuted">From<input type="date" className={`${inputClass} mt-1`} value={customFrom} onChange={e => setCustomFrom(e.target.value)} /></label><label className="text-sm font-bold text-textMuted">To<input type="date" className={`${inputClass} mt-1`} value={customTo} onChange={e => setCustomTo(e.target.value)} /></label></>}</div>
