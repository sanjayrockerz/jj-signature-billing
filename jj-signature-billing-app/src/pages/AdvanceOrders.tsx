...
import { buildAdvanceDepositWhatsAppMessage, buildProfessionalWhatsAppMessage, publicInvoiceUrl } from '../lib/whatsappMessageUtf8'
...
function Field({ label, children }: { label: string; children: React.ReactNode }) { return <label className="block"><span className="mb-1.5 block text-sm font-black uppercase tracking-wide text-[#6B7280]">{label}</span>{children}</label> }
const inputClass = 'w-full rounded-xl border border-[#D8CBB7] bg-[#FFFDF8] px-3.5 py-2.5 text-sm text-[#111111] outline-none transition focus:border-[#CBB89D] focus:ring-2 focus:ring-[#CBB89D]/30'
...
export default function AdvanceOrders({ onOrderCompleted }: AdvanceOrdersProps = {}) {
  ...
  const create = async (event: FormEvent) => {
    ...
    if (!normalizePhone(form.phone.trim())) { setError('Please enter a valid Indian mobile number (e.g. 6379048966 or +91 6379048966).'); setSaving(false); return }
    try {
  ...
  const receivePayment = async (event: FormEvent) => {
    ...
    try {
      ...
      if (appliedCoupon) parts.push(`Coupon: ${appliedCoupon.code} (-${appliedCoupon.percentage}%) = -${formatCurrency(couponDiscount)}`)
      if (manualDisc > 0) parts.push(`Manual Discount: ${manualDiscountType === '%' ? manualDiscountNum + '%' : formatCurrency(manualDiscountNum)} = -${formatCurrency(manualDisc)}`)
  ...
  const invoiceFile = async (order: AdvanceOrder) => invoicePdfFile({ invoiceNo: order.invoice_number || order.deposit_id, date: order.completed_at || new Date().toISOString(), customerName: order.customer_name, phone: order.phone, address: order.address, items: productRows(order), subtotal: order.total_amount, shipping: 0, total: order.total_amount, paymentMode: order.final_payment_method || 'Paid' })
  ...
  const cards = [
    ...
    ['Total Deposit Amount', formatCurrency(analytics.deposits), RupeeIcon, 'text-black bg-[#EDE4D4]'], ['Outstanding Balance', formatCurrency(analytics.outstanding), RupeeIcon, 'text-black bg-[#EDE4D4]'],
  ...
  return <div className="advance-orders-content space-y-5">
    <div className="hidden space-y-3 md:hidden">
      {loading ? <div className="rounded-2xl border border-[#ECE9E2] bg-white p-8 text-center text-sm text-[#6B7280]">Loading advance orders...</div> : filtered.length === 0 ? <div className="rounded-2xl border border-[#ECE9E2] bg-white p-8 text-center text-sm text-[#6B7280]">No advance orders match these filters.</div> : filtered.map(order => <article key={`mobile-${order.id}`} className="rounded-2xl border border-[#ECE9E2] bg-white p-4 shadow-sm"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="font-black text-violet-700">{order.deposit_id}</p><p className="mt-1 break-words text-base font-bold text-[#273126]">{order.customer_name}</p><p className="text-xs text-[#727970]">{order.phone}</p></div><span className={`shrink-0 rounded-lg px-2 py-1 text-xs font-black uppercase ${STATUS_STYLES[order.status]}`}>{STATUS_LABELS[order.status]}</span></div><div className="mt-3 grid grid-cols-2 gap-2 text-xs"><div className="rounded-xl bg-[#F8F7F4] p-3"><p className="font-black uppercase text-[#858C83]">Product</p><p className="mt-1 break-words font-bold">{order.product_name}</p></div><div className="rounded-xl bg-[#F8F7F4] p-3"><p className="font-black uppercase text-[#858C83]">Delivery</p><p className="mt-1 font-bold">{new Date(`${order.expected_delivery_date}T00:00:00`).toLocaleDateString('en-IN')}</p></div><div className="rounded-xl bg-[#F8F7F4] p-3"><p className="font-black uppercase text-[#858C83]">Total</p><p className="mt-1 font-bold">{formatCurrency(order.total_amount)}</p></div><div className="rounded-xl bg-[#F8F7F4] p-3"><p className="font-black uppercase text-[#858C83]">Balance</p><p className="mt-1 font-bold text-red-600">{formatCurrency(order.remaining_balance)}</p></div></div><div className="mt-3 flex flex-wrap gap-2"><button onClick={() => void openDetails(order)} className="min-h-11 flex-1 rounded-xl bg-[#F4F2F6] px-3 py-2 text-xs font-black text-violet-700">Details</button>{order.status !== 'completed' && order.status !== 'cancelled' && <button onClick={() => setPaymentOrder(order)} className="min-h-11 flex-1 rounded-xl bg-emerald-600 px-3 py-2 text-xs font-black text-white">Receive</button>}{order.status !== 'completed' && <><button onClick={() => printAdvanceReceipt(order)} className="min-h-11 rounded-xl bg-amber-50 px-3 py-2 text-amber-700" title="Print advance receipt"><Printer size={17}/></button><button onClick={() => downloadFile(advanceReceiptPdf(order))} className="min-h-11 rounded-xl bg-amber-50 px-3 py-2 text-amber-700" title="Download advance receipt"><Download size={17}/></button></>}{order.status === 'completed' && <><button onClick={() => downloadFile(invoiceFile(order))} className="min-h-11 rounded-xl bg-violet-50 px-3 py-2 text-violet-700" title="Download invoice"><Download size={17}/></button><button onClick={() => printFinal(order)} className="min-h-11 rounded-xl bg-violet-50 px-3 py-2 text-violet-700" title="Print receipt"><Printer size={17}/></button></>}</div></article>)}
    </div>
    ...
    <StatGrid>{cards.map(([label, value, Icon, color]) => { const [iconColor, iconBg] = color.split(' '); return <StatCard key={label} label={label} value={value} icon={<Icon size={21}/>} color={iconColor} bg={iconBg} className="border-[#ECE9E2]" labelClassName="text-[#879086]" valueClassName="text-2xl text-[#273126]" /> })}</StatGrid>
